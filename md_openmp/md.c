...
double comp_accel() {
	...
	#pragma omp parallel for collapse(2) reduction(+:pot_energy)
	for (int i = 1; i < x+1; i++) {
		for (int j = 1; j < y+1; j++) {
...
int main(int argc, char *argv[]) {
	...
	// calculate value outside loop to be used for temp calculation
	...
	//printf("Total CPU time: %lf seconds/n", total_cpu_time);
